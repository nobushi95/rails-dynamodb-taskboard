...
### User の SK に `METADATA` を使う理由

複合キー（PK + SK）のテーブルでは、**すべてのアイテムに SK の値が必須**である。SK を空にすることはできない。

User エンティティは1ユーザーにつき1アイテムなので、本来 SK でデータを区別する必要がない。しかし複合キーテーブルである以上、何らかの値を入れる必要がある。そこで `"METADATA"` という固定文字列を**プレースホルダ**として使っている。

シングルテーブル設計では、同じ PK パーティション内に複数種類のアイテムが同居するため、SK の値でエンティティを区別する役割も持つ:

```
PK = "USER#user1"
  SK = "METADATA"        ← ユーザー自身の情報
  SK = "PROJECT#proj1"   ← このユーザーのプロジェクト
  SK = "PROJECT#proj2"   ← このユーザーのプロジェクト
```

> **補足:** `METADATA` という名前は慣習的なもので、DynamoDB の仕様ではない。`PROFILE` や `INFO` など任意の固定値で構わない。「このアイテムはエンティティ自体のメタ情報である」という意図を表す命名として広く使われている。
...
```mermaid
flowchart LR
  subgraph Base["ベーステーブル"]
    direction TB
    B_PK["pk: PROJECT#proj1"]
    B_SK["sk: TASK#2026-03-01#task1"]
    B_ATTR["title, status, assignee_id, ..."]
    B_GSI1["gsi1pk: STATUS#done"]
    B_GSI2["gsi2pk: ASSIGNEE#user1"]
  end

  subgraph GSI1["GSI1 (ステータス別)"]
    direction TB
    G1_PK["gsi1pk: STATUS#done"]
    G1_SK["gsi1sk: 2026-03-01#task1"]
    G1_ATTR["全属性のコピー (projection: ALL)"]
  end

  subgraph GSI2["GSI2 (担当者別)"]
    direction TB
    G2_PK["gsi2pk: ASSIGNEE#user1"]
    G2_SK["gsi2sk: 2026-03-01#task1"]
    G2_ATTR["全属性のコピー (projection: ALL)"]
  end
